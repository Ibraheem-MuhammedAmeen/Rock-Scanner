...
import '../../model/mineral_model.dart';

class MineralRock extends StatefulWidget {
  const MineralRock({super.key});

  @override
  State<MineralRock> createState() => _MineralRockState();
}

class _MineralRockState extends State<MineralRock> {
  late final Box<MineralRockModel> rockBox;

  @override
  void initState() {
    super.initState();
    rockBox = Hive.box<MineralRockModel>('MineralRocks');
    _populateRocksIfNeeded();
  }

  void _populateRocksIfNeeded() {
    if (rockBox.isEmpty) {
      final igneousRocks = <MineralRockModel>[
        MineralRockModel(
          name: 'Granite',
          desc:
